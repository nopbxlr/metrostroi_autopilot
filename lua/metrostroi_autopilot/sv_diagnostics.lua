...
function AI.CmdTermDebug(ply)
    ...
    line("=== !ai term  [build=facemap9] ===")   -- if you don't see this tag, the server is running OLD lua
    ...
    for _, sw in ipairs(ents.FindByClass("gmod_track_switch")) do
        if IsValid(sw) then
            ...
            if math.abs(fd) < 450 * AI.U_PER_M and lat < 1000 then
                n = n + 1
                local trailing, hdot = drv:SwitchTrailing(sw)
                line(string.format("  switch %s  fwd=%+.0fm  lat=%.0fu  alt=%s  %s(hdot=%s)",
                    tostring(sw:GetNW2String("ID", "?")), fd / AI.U_PER_M, lat, tostring(sw.AlternateTrack),
                    trailing and "TRAILING" or "facing", hdot and string.format("%+.2f", hdot) or "?"))
